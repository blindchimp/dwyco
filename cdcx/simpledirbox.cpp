...
SimpleDirBox::SimpleDirBox(QWidget *parent) :
    WHATBOX(parent)
{
    setWindowTitle("Recent Visitors");
}

void
SimpleDirBox::create_new_url()
{
    const char *auth;
    int len;
...
#ifdef LOCAL_TEST
    url.setUrl("http://localhost/cgi-bin/mksimphtdir.sh");
#else
#ifndef CDCX_NO_SSL
...
#endif
    // note: this needs to be updated for qt5
#ifdef DWYCO_QT5
    QUrlQuery qurl;
