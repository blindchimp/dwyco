...
* The randobot is constantly checking for users that have sent in more randos than they have received. If it finds a user that has sent in more randos than they have received, it does the following:
	* Picks a rando more or less at random that the user has not received before.
	* Sends that rando directly to the user, the message will contain the approximate location of the rando.
	* Sends another auxilliary message to the CREATOR of the rando we just sent. The message contains the hash of the picture, and the location of the user we just sent the rando to.
