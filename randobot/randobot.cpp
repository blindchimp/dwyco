...
uid_due_randos()
{
    ...
    // testing D->sql_simple("delete from c1 where from_uid in (select to_uid from sent_to where strftime('%s', 'now') - time < 3600 group by to_uid having(count(*) > 2));");
            ...
            if(Iplog)
            {
                ...
                try {
                    ...
                    if(res.num_elems() == 1 && res[0][0].type() == VC_STRING && res[0][0].len() > 0)
                    {
                        ...
                        //QByteArray cmd = group_msg((const char *)hash, (const char *)res[0][0]);
