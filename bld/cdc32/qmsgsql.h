...
int sql_uid_all_mid_tagged(const vc &uid, const vc &tag);
vc sql_get_all_idx();
...
int sql_is_mid_local(const vc &mid);
int sql_is_mid_anywhere(const vc &mid);
int sql_mid_has_tombstone(const vc &mid);
...
void add_pull_failed(const vc &mid, const vc &uid, const vc& why);
