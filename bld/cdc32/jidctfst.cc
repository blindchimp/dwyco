...
/*
 * $Header: g:/dwight/repo/cdc32/rcs/jidctfst.cc 1.3 1997/11/25 20:41:03 dwight Stable095 $
 */
/*
 * jidctfst.c
 *
...
#if 0
IFAST_MULT_TYPE FDct_table[DCTSIZE2];
IFAST_MULT_TYPE IDct_table[DCTSIZE2];
void
gen_aan_scales()
{
...
extern JQUANT_TBL *Quant_tbl_ptrs[];
extern int jpeg_zigzag_order[];
	for (int i = 0; i < DCTSIZE2; i++) {
	  FDct_table[i] = (IFAST_MULT_TYPE)
	    DESCALE((INT32) aanscales[i] * Quant_tbl_ptrs[0]->quantval[jpeg_zigzag_order[i]],
		    aan_CONST_BITS-3);
	if(FDct_table[i] == 0) FDct_table[i] = 1;
	  IDct_table[i] = (IFAST_MULT_TYPE)
	    DESCALE((INT32) aanscales[i] * Quant_tbl_ptrs[0]->quantval[jpeg_zigzag_order[i]],
...
jpeg_idct_ifast (
		 JCOEFPTR coef_block,
		 T** output_buf, int output_col, QTAB* qtab, int do_limit)
{
    ...
    if ((wsptr[1] | wsptr[2] | wsptr[3] | wsptr[4] | wsptr[5] | wsptr[6] |
	 wsptr[7]) == 0) {
      /* AC terms all zero */
		T dcval;
		if(do_limit)
			dcval = range_limit[IDESCALE(wsptr[0], PASS1_BITS+3)
				  & RANGE_MASK];
		else
			dcval = IDESCALE(wsptr[0], PASS1_BITS+3);
