...
{
    if(on)
        Activity_timer.start(false, timeout * 1000);
    else
        Activity_timer.stop();
...
db_reconnect()
{
    ...
    if(Database_id == -1)
    {
        if(!been_here)
        {
            Db_timer.start(false, 1);
            been_here = 1;
        }
        if(!Db_timer.is_running())
        {
            Db_timer.start(false, (10 + (dwyco_rand() % 45)) * 1000);
        }
...
handle_deferred_msg_send()
{
    ...
    {
        been_here = 1;
        send_qd_msg_timer.start(true, 10 * 1000, 10 * 1000);
        qd_send_one();
    }
...
#undef EMERGENCY_TEST
...
#ifdef DW_RTLOG
    {
        ...
        if(!been_here)
        {
            dump_timer.start(true, 5000, 5000);
            been_here = 1;
        }
...
group_enter_setup(vc m, void *, vc, ValidPtr vp)
{
    ...
    try
    {
        ...
        if(what[0] == vc("exists"))
        {
            ...
            if(!start_gj(members[0], dha->alt_name(), DH_alternate::Group_join_password))
                throw -1;
        }
