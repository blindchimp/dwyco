...
gray **
pgm_allocarray(int cols, int rows)
{
...
void
pgm_freearray(gray **arr, int rows)
{
    if (!arr) return;
...
#if 0
gray **
pgm_readpgm(FILE *fp, int *colsP, int *rowsP)
{
    if (!fp) return NULL;
    char format[3];
...
int
pgm_writepgm(FILE *fp, gray **img, int cols, int rows)
{
    if (!fp) return 0;
    int maxval = 255;
    fprintf(fp, "P5\n%d %d\n%d\n", cols, rows, maxval);
    for (int r = 0; r < rows; r++)
    {
        if (fwrite(img[r], 1, cols, fp) != (size_t)cols)
        {
            return 0;
        }
    }
