...
vc
vclh_putenv(const vc& name, const vc& value)
{
    const char *put;
...
vc
vclh_file_remove(const vc& file)
{
...
vc
vclh_file_exists(const vc& file)
{
	if(file.type() != VC_STRING)
...
vc
vclh_file_access(const vc& file, const vc& how)
{
	if(file.type() != VC_STRING)
...
// the operation.
vc
vclh_file_rename(const vc& from, const vc& to)
{
...
vc
vclh_strftime(const vc& time, const vc& format)
{
	if(time.type() != VC_INT)
...
vc
vclh_strftime_hp(const vc& format)
{
#ifdef UNIX
	if(format.type() != VC_STRING)
		USER_BOMB("second arg to strftime must be format specifier string", vcnil);
...
vc
vclh_kill(const vc& pid, const vc& sig)
{
#ifndef __WIN32__
