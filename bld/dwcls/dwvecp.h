...
public:
	...
	int operator!=(const DwVecP& t) const {return !(*this == t);}

	T*& operator[](long idx) const {return (T*&)DwVec<void *>::operator[](idx);}
	T*& operator[](long idx) {return (T*&)DwVec<void *>::operator[](idx);}

	long num_elems() const {return DwVec<void *>::num_elems();}
	void set_size(long count) {DwVec<void *>::set_size(count);}
	void append(const T* e) {DwVec<void *>::append((void *)e);}
	void insert(const T* e, long i) {DwVec<void *>::insert((void *)e, i);}
	void spread(long idx, long cnt = 1) {DwVec<void *>::spread(idx, cnt);}
	void del(long idx, long cnt = 1) {DwVec<void *>::del(idx, cnt);}
	long index(const T* e) const {return DwVec<void *>::index((void *)e);}
	int contains(const T* e) const {return DwVec<void *>::contains((void *)e);}
	void apply(Funcp f) {DwVec<void *>::apply((DwVec<void *>::Funcp)f);}

	void* get_by_iter(DwIter<DwVec<void *>, void *> *a) const {
		return (T*)DwVec<void *>::get_by_iter(a);
	}
#ifdef DWVEC_DOINIT
	void init_value(void *&v) {v = 0;}
#endif
};
...
public:
	...
	T* getp() {return (T*)to_iterate->get_by_iter(this);}
};
