...
public:
	DwAMapIter(const DwAMap<R,D> *t) : DwMapsIter<R,D>(t) {dmi = t->map->make_iter();}
	~DwAMapIter() { delete dmi; }
	void init() {dmi->init();}
	void rewind() {dmi->rewind();}
	void fast_forward() {dmi->fast_forward();}
	void forward() {dmi->forward();}
	void backward() {dmi->backward();}
	int eol() {return dmi->eol();}
};
