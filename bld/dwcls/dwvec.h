...
public:
	...
	int operator!=(const DwVec& t) const {return !(*this == t);}
	...
	long num_elems() const {return count;}
...
#endif
#ifdef DWVEC_DOINIT
	void init_value(T& v) { if(initfun) (*initfun)(v); else v = T(); }
#endif
};
...
public:
	DwVecIter(const DwVec<T> *t) : DwIter<DwVec<T>, T>(t) { cur = 0; max = t->num_elems();}
	...
	// compilers.
	void init() {cur = 0; max = DwIter<DwVec<T>, T>::to_iterate->num_elems() - 1;}
	void rewind() {cur = 0;}
	void fast_forward() {cur = DwIter<DwVec<T>, T>::to_iterate->num_elems() - 1;}
	void forward() {if(!(cur >= max)) ++cur; }
	void backward() {if(!(cur < 0)) --cur;}
	int eol() {return cur < 0 || cur >= max; }
};
