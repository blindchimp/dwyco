...
// keep gcc4 from taking a shit on itself
thdr
tcls::~DwMapR()
{
}
...
{
#ifdef CFRONT
	DwMapRIter<R,D> *mip = (DwMapRIter<R,D> *)a;
#else
...
public:
	...
	~DwMapRIter() {}
	void init() {iter.init();}
	void rewind() {iter.rewind();}
	void fast_forward() {iter.fast_forward();}
	void forward() {iter.forward();}
	void backward() {iter.backward();}
	int eol() {return iter.eol();}
};
