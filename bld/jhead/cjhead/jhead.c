...
static const  int TrimExif = FALSE;        // Cut off exif beyond interesting data.
static const  int RenameToDate = 0;        // 1=rename, 2=rename all.
#ifdef _WIN32
static const  int RenameAssociatedFiles = FALSE;
#endif
static const  char * strftime_args = NULL; // Format for new file name.
static const  int Exif2FileTime  = FALSE;
       int ShowTags     = FALSE;    // Do not show raw by default.
static const  int Quiet        = FALSE;    // Be quiet on success (like unix programs)
       int DumpExifMap  = FALSE;
static const  int ShowConcise  = FALSE;
static const  int CreateExifSection = FALSE;
static const  int TrimExifTrailingZeroes = FALSE;
static const  char * ApplyCommand = NULL;  // Apply this command to all images.
static const  char * FilterModel = NULL;
static const  int    ExifOnly    = FALSE;
static const  int    PortraitOnly = FALSE;
static const  time_t ExifTimeAdjust = 0;   // Timezone adjust
static const  time_t ExifTimeSet = 0;      // Set exif time to a value.
static   char DateSet[11];
static const  unsigned DateSetChars = 0;
static const  unsigned FileTimeToExif = FALSE;

// just remove as much as possible
static const  int DeleteComments = TRUE;
static const  int DeleteExif = TRUE;
static const  int DeleteIptc = TRUE;
static const  int DeleteXmp = TRUE;
static const  int DeleteUnknown = TRUE;

static const  char * ThumbSaveName = NULL; // If not NULL, use this string to make up
                                    // the filename to store the thumbnail to.

static const  char * ThumbInsertName = NULL; // If not NULL, use this string to make up
                                    // the filename to retrieve the thumbnail from.

static const  int RegenThumbnail = FALSE;

static const  char * ExifXferScrFile = NULL;// Extract Exif header from this file, and
                                    // put it into the Jpegs processed.

static const  int EditComment = FALSE;     // Invoke an editor for editing the comment
static const  int SupressNonFatalErrors = FALSE; // Wether or not to pint warnings on recoverable errors

static const  char * CommentSavefileName = NULL; // Save comment to this file.
static const  char * CommentInsertfileName = NULL; // Insert comment from this file.
static const  char * CommentInsertLiteral = NULL;  // Insert this comment (from command line)

static const  int AutoRotate = TRUE;
static const  int ZeroRotateTagOnly = FALSE;

static const  int ShowFileInfo = TRUE;     // Indicates to show standard file info
...
static int DoAutoRotate(const char * FileName)
{
    if (ImageInfo.Orientation != 1){
        Rotation_found =  ClearOrientation();
        return TRUE;
#if 0
...
static void ProcessFile(const char * FileName)
{
    ...
    if (DoModify){
        ReadMode = (ReadMode_t)((int)READ_IMAGE|(int)ReadMode);
    }
...
int do_jhead (const char *filename)
{
try{
    int argn;
    char * arg;
    ...
    if (FileSequence == 0){
        ...
        return EXIT_SUCCESS | (Rotation_found << 1);
    }
	}
	catch(int)
	{
